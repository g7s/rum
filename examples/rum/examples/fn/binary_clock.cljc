(ns rum.examples.fn.binary-clock
  (:require
    [rum.core :as rum]
...
(rum/defnc render-count [ref]
  [:div.stats "Renders: " (rum/use-react ref)])


(rum/defnc bit [n bit]
  (rum/use-effect #(swap! *bclock-renders inc))
  [:td.bclock-bit {:style (when (bit-test n bit) {:backgroundColor @core/*color}) }])


(rum/defnc binary-clock []
  (let [ts   (rum/use-react core/*clock)
        msec (mod ts 1000)
